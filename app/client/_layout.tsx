...
export function CustomTabBar({ state, descriptors, navigation, insets }: BottomTabBarProps) {
    ...
    return (
        <View style={{ flexDirection: 'row', marginBottom: insets.bottom, justifyContent: "center", position: "relative" }}>
            <View style={{ zIndex: 10 }} className="relative bg-primary-500  flex-row gap-x-4 p-2 rounded-full">
                {Object.values(descriptors).map(({ route, options }, index) => {
                    const isFocused = state.index === index;
                    const icon = typeof options.tabBarIcon == "function" ? options.tabBarIcon({ focused: isFocused, color: isFocused ? theme.extend.colors.primary.DEFAULT : theme.extend.colors.gray[100], size: 20 }) : <></>
                    ...
                    return (
                        <Pressable onPress={onPress} onLongPress={onLongPress} key={index} className={clx("w-[47.5px] bg-primary-300 justify-center items-center h-[45px] rounded-full ", index == 1 && "mr-[60px]", isFocused && "bg-transparent")} >
                            <View >
                                {icon}
                            </View>
                        </Pressable>
                ...
                <Animated.View
                    ...
                    className={"w-[47.5px] h-[45px]   bg-white rounded-full absolute "}
                />
            </View>
            <View className="absolute left-1/2 -translate-x-1/2 -translate-y-1/4" style={{ zIndex: 15 }} >
                <Pressable className=" relative w-[60px] h-[60px] justify-center items-center bg-primary-200 rounded-full">
...
function useLinkBuilder(): { buildHref: any; } {
