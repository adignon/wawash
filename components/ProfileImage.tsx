...
export function ProfileImage({ imageUrl, icon, width = 120, height = 120, borderColor = theme.extend.colors.gray.DEFAULT, borderWidth = 1 }: { imageUrl?: string | null, icon?: any, width?: number, height?: number, borderColor?: string, borderWidth?: number }) {
    ...
    return (
        <View className="border border-gray relative  rounded-full bg-white dark:bg-dark-lighter dark:border-dark-border items-center justify-center  m-3" style={{ width, height, borderWidth: borderColor ? borderWidth : undefined }}>
            {
                (imageUrl) ?
                    <Image
