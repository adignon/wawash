            ...
            },
            {
                ...
                description: (
                    <View className="flex-row items-center gap-x-2">
                        ...
                        {
                            ["STARTED", "PICKED", "WASHING"].includes(order.status) || order.userKg &&
                            <>
            ...
            },
            {
                ...
                icon: (
                    <Svg width="17" height="21" viewBox="0 0 17 21" fill="none" >
                        <Path d="M14.7498 0.0937602L2.24984 0.0833435C1.09359 0.0833435 0.166504 1.01043 0.166504 2.16668V18.8333C0.166504 19.9896 1.09359 20.9167 2.24984 20.9167H14.7498C15.9061 20.9167 16.8332 19.9896 16.8332 18.8333V2.16668C16.8332 1.01043 15.9061 0.0937602 14.7498 0.0937602ZM14.7498 18.8333H2.24984L2.23942 2.16668H14.7498V18.8333Z" fill={["READY", "DELIVERED"].includes(order.status) ? "white" : (!!order.userKg ? (colorScheme == "light" ? theme.extend.colors.green[400] : theme.extend.colors.green[400]) : (colorScheme == "light" ? theme.extend.colors.dark.DEFAULT : theme.extend.colors.gray[200]))} />
                        <Path d="M4.33317 5.29168C4.90847 5.29168 5.37484 4.82531 5.37484 4.25001C5.37484 3.67471 4.90847 3.20834 4.33317 3.20834C3.75787 3.20834 3.2915 3.67471 3.2915 4.25001C3.2915 4.82531 3.75787 5.29168 4.33317 5.29168Z" fill={["READY", "DELIVERED"].includes(order.status) ? "white" : (!!order.userKg ? (colorScheme == "light" ? theme.extend.colors.green[400] : theme.extend.colors.green[400]) : (colorScheme == "light" ? theme.extend.colors.dark.DEFAULT : theme.extend.colors.gray[200]))} />
                        <Path d="M7.45817 5.29168C8.03347 5.29168 8.49984 4.82531 8.49984 4.25001C8.49984 3.67471 8.03347 3.20834 7.45817 3.20834C6.88287 3.20834 6.4165 3.67471 6.4165 4.25001C6.4165 4.82531 6.88287 5.29168 7.45817 5.29168Z" fill={["READY", "DELIVERED"].includes(order.status) ? "white" : (!!order.userKg ? (colorScheme == "light" ? theme.extend.colors.green[400] : theme.extend.colors.green[400]) : (colorScheme == "light" ? theme.extend.colors.dark.DEFAULT : theme.extend.colors.gray[200]))} />
                        <Path d="M8.49984 17.7917C11.3748 17.7917 13.7082 15.4583 13.7082 12.5833C13.7082 9.70834 11.3748 7.37501 8.49984 7.37501C5.62484 7.37501 3.2915 9.70834 3.2915 12.5833C3.2915 15.4583 5.62484 17.7917 8.49984 17.7917ZM10.9582 10.125C12.3123 11.4792 12.3123 13.6875 10.9582 15.0417C9.604 16.3958 7.39567 16.3958 6.0415 15.0417L10.9582 10.125Z" fill={["READY", "DELIVERED"].includes(order.status) ? "white" : (!!order.userKg ? (colorScheme == "light" ? theme.extend.colors.green[400] : theme.extend.colors.green[400]) : (colorScheme == "light" ? theme.extend.colors.dark.DEFAULT : theme.extend.colors.gray[200]))} />
                    </Svg>

                ),
                isActive: !!order.userKg,
            ...
            },
            {
                ...
                description: (
                    <View className="flex-row items-center gap-x-2">
                        <Text className="font-jakarta text-[14px] text-dark-300 dark:text-gray-400">{`${order.pickingHours[0]} - ${order.pickingHours[1]}`}</Text>
                        {
                            ["STARTED", "PICKED", "WASHING"].includes(order.status) &&
                            <>
                                <View className="w-[3px] h-[3px] rounded-full bg-dark-300 dark:bg-gray"></View>
                                <Text className="font-jakarta text-[14px] text-dark-300 dark:text-gray-400">{order.userKg ? `${order.userKg}Kg collectés` : t("En cours")}</Text>
                            </>
                ...
                isActive: !!order.userKg && ["READY", "DELIVERED"].includes(order.status),
                        ...
                        <View>
                            {
                                order.invoiceId && (
                                    <View className="p-4 gap-y-6">
                                        <Text className="font-jakarta-semibold text-dark-400 dark:text-gray-100 text-[16px] border-b border-gray dark:border-dark-400 pb-4">{t("Factuation kg additionnel")}</Text>
                ...
                <ActionModal show={show} title={(
                    ...
                    <View className="my-10 gap-y-4 ">
                        <View className="flex-row items-center justify-between">
                            ...
                            <Text className="text-[16px] font-jakarta-medium text-primary dark:text-primary-500 ">{`${order.customerOrderKgPrice}f/Kg`}</Text>
                        </View>
                        <View className="flex-row items-center justify-between">
                            <Text className="text-[16px] font-jakarta-semibold text-dark dark:text-dark-200 ">{t("Prix du kg")}</Text>
                            <Text className="text-[16px] font-jakarta-semibold text-primary dark:text-primary-500 ">{`${order.add_price}f/Kg`}</Text>
                        </View>
                    </View>
                    {
                        true && (
                            <View className="">
                                <Button.Primary
                                    ...
                                    label={
                                        <Text className="font-jakarta-semibold text-[16px] text-white">{t("Payer - " + order.add_price + "f")}</Text>
                                    }
                                />
                            </View>
                        )
...
export function StatusInvoice({ invoice, contained }: { invoice?: IInvoice, contained?: boolean }) {
    const { colorScheme } = useColorScheme()
