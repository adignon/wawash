...
export function Packages({ filterPackage, showActive = true, allowSubscriptionCancel = false, onPackageSelect }: { allowSubscriptionCancel?: boolean, filterPackage?: (x: IPackage[]) => IPackage[], showActive?: boolean, onPackageSelect?: () => void }) {
    ...
    console.log("active", showActive, user?.activeSubscription)
    ...
    }, [user?.activeSubscription?.id, packages.data])
    React.useEffect(() => {
        if (packages.data && user?.activeSubscription?.id != packages.data?.subscription?.id) {
            setUser({
                ...user,
                activeSubscription: packages.data?.subscription
            } as any)
        }
    }, [packages.data, user?.activeSubscription])


    const data = useMemo(() => {
        const data = packages.data ? packages.data.packages.sort((a: any, b: any) => a.name.localeCompare(b.name, "fr", { sensitivity: "base" })) : []
